...
serve(async (req) => {
  ...
  try {
    console.log('🚀 INSTAGRAM: Iniciando...')
    ...
    // PASO 2: Buscar páginas con Instagram
    console.log('🔍 Buscando Instagram Business Account...')
    const accountsResponse = await fetch(
      `https://graph.facebook.com/v18.0/me/accounts?access_token=${igToken}&fields=instagram_business_account{id,username}`
    )
    
    if (!accountsResponse.ok) {
      const accountsError = await accountsResponse.text()
      console.error('❌ Error en cuentas:', accountsError)
      throw new Error(`Error obteniendo cuentas: ${accountsError}`)
    }
    ...
    if (!accountsData.data || accountsData.data.length === 0) {
      throw new Error('No se encontraron páginas de Facebook con Instagram conectado')
    }
    
    // PASO 3: Encontrar cuenta de Instagram
    let instagramAccount = null
    
    for (const account of accountsData.data) {
      if (account.instagram_business_account) {
        instagramAccount = account.instagram_business_account
        console.log('✅ Instagram encontrado:', instagramAccount.username || instagramAccount.id)
        break
      }
    }
    
    if (!instagramAccount) {
      throw new Error('No hay Instagram Business conectado. Conecta tu Instagram en facebook.com/settings')
    }
    
    // PASO 4: Obtener media de Instagram
    console.log('📸 Obteniendo posts de Instagram...')
    const mediaResponse = await fetch(
      `https://graph.facebook.com/v18.0/${instagramAccount.id}/media?fields=id,caption,media_type,media_url,thumbnail_url,permalink,timestamp,like_count,comments_count&limit=20&access_token=${igToken}`
    )
    
    if (!mediaResponse.ok) {
      const mediaError = await mediaResponse.text()
      console.error('❌ Error en media:', mediaError)
    ...
    if (videos.length === 0) {
      console.warn('⚠️ No se encontraron videos en la cuenta')
      return new Response(
        JSON.stringify({ data: [] }),
    ...
    const reels = await Promise.all(
      videos.slice(0, 8).map(async (reel: any) => {
        ...
        try {
          ...
          console.warn('⚠️ Sin comentarios para', reel.id)
        }
    ...
    console.log(`✅ ¡Éxito! ${reels.length} reels procesados`)
    ...
    return new Response(
      JSON.stringify({ 
        error: error.message,
        hint: 'Verifica que TOKEN_IG_TEST esté en Supabase Secrets y que tu Instagram esté conectado a Facebook'
      }),
      { 
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 500 
